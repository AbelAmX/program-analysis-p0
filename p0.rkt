...
(define (interp-hw1 e env)
  ...
  (match e
    ...
    [`(lambda (,vars ...) ,body)
     ...
     (lambda l 
        (let ([env-copy (make-hash
                        (hash-map env
                                  (lambda (x y) (cons x y))))])
          (define (check-list var lis)
            (if (empty? var)
                env-copy
                (begin
                 (hash-set! env-copy (first var) (first lis))
                 (check-list (rest var) (rest lis)))))
         (interp-hw1 body (check-list vars l)))) ]
    ;; TODO: implement let* correctly
    [`(let* ([,(? symbol? li) ,(? expr? lbody)] ...)
        ,(? expr? body))
     (define (let-seq var exp )
       (if (empty? var)
            env
            (let* ([vars (car var)]
                   [exps (car exp)]
                   [b #f]
                   [inp (begin (hash-set! env vars b) ((lambda (x) (interp-hw1 x env)) exps))])
                   (begin
                      (hash-set! env vars inp)
                      (let-seq (cdr var) (cdr exp))))))
     (interp-hw1 body (let-seq li lbody))]
    ;; TODO: handle multi-argument letrec (still only one f)
    [`(letrec ([,(? symbol? f) (lambda (,f-args ...) ,f-body)])
        ,(? expr? body))
     (define (check-l var lis envx)
                                 (if (empty? var)
                                     envx
                                     (begin
                                       (hash-set! envx (car var) (car lis))
                                       (check-l (cdr var) (cdr lis) envx))))
     ;; Make a mutable copy of the hash table
     (let ([env-copy (make-hash
                      (hash-map env
                                (lambda (x y) (cons x y))))])
       ...
       (hash-set! env-copy
                  f
                  (lambda x (interp-hw1 f-body (check-l f-args x env-copy) )))
       ;; Now interpret the body with this updated (mutable) env
    ...
    [`(,(? expr? e0) ,(? expr? e-args) ...)
     (apply (interp-hw1 e0 env) (map (lambda (x) (interp-hw1 x env)) e-args))]))
...
(define (eval-match-statements patterns bodies matching-value env)
  ...
  (define (compile-match-statement pattern env)
    (match pattern
      ...
      [`(list ,patterns ...)
       (define (check-l patt lis leng envx)
         (let ([env-lis ((compile-match-statement (car patt) envx) (car lis) )]
               [len (- leng 1)])
              (if env-lis
                  (if (= len 0)
                      env-lis
                      (check-l (cdr patt) (cdr lis) len env-lis))  
                   #f)))
       (lambda (x)
         (if (and (list? x) (equal? (length x) (length patterns)))
             (check-l patterns x (length patterns) env)
             #f))]))
